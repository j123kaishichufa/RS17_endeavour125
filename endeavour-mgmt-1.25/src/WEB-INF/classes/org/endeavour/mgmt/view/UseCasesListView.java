...
import org.endeavour.mgmt.view.model.UseCasesListModel;
...
public class UseCasesListView extends PanelComponent implements ActionListener, Observer {

	private UseCaseMaintenance useCaseMaintenance = null;
	private GridBoxComponent useCasesGrid = null;
	private Button newButton = null;
	private Button editButton = null;
	private Button deleteButton = null;
	private UseCasesListModel useCasesModel = null;
	...
	public UseCasesListView() {
		...
		theLogo.getStyle().getBackground().setImage(IViewConstants.USE_CASES_ICON);
		theHeaderPanel.add(theLogo.setLimit("0, 0"));

		Label theHeaderLabel = new Label(IViewConstants.RB.getString("use_cases.lbl"));
		...
		this.statusModel = new StatusListModel(false, true);
		...
		Label theNumberLabel = new Label(IViewConstants.RB.getString("use_case_number.lbl"));
	...
	private void setButtonsStatus() {
		...
		this.editButton.setEnabled(isEnabled && !this.useCasesGrid.getRows().isEmpty());
		...
		this.deleteButton.setEnabled(isEnabled && !this.useCasesGrid.getRows().isEmpty() && hasPrivilege);
	...
	private void initializeControllers() {
		...
		this.useCaseMaintenance = new UseCaseMaintenance(theProjectId != null ? theProjectMaintenance.retrieveProjectBy(theProjectId) : null);
		this.useCaseMaintenance.addObserver(this);
	}

	private void initializeUseCasesGrid() {
		this.useCasesModel = new UseCasesListModel();
		this.useCasesGrid = new GridBoxComponent(this.useCasesModel);
		this.useCasesGrid.addActionListener(GridBox.ACTION_DOUBLE_CLICK, this);
		this.useCasesGrid.setColumnWidth(0, 50);
		this.useCasesGrid.setColumnWidth(1, 170);
		super.add(this.useCasesGrid.setLimit("0, 2"));
	}

	private void viewUseCases() {
		...
		this.useCasesModel.setData(this.useCaseMaintenance.getUseCasesBy(thePriority, theStatus, theName, theNumber));
		Integer theId = this.useCaseMaintenance.getSelectedWorkProductId();
		if (theId != null) {
			this.useCasesGrid.setSelectedRowById(theId);
		}
		this.setButtonsStatus();
	}
	...
	public void actionPerformed(ActionEvent aEvt) {
		...
		if (theSource instanceof Button) {
			...
			if (theButton.getText().equals(IViewConstants.NEW_BUTTON_LABEL)) {
				new UseCaseView(this.useCaseMaintenance);
			}
	...
	private void viewUseCase() {
		Integer theUseCaseId = this.useCasesGrid.getSelectedRowId();
		if (theUseCaseId != null) {
			this.useCasesGrid.setEnabled(false);
			new UseCaseView(theUseCaseId, this.useCaseMaintenance);
			this.useCasesGrid.setEnabled(true);
		}
	}

	private void deleteUseCase() {
		this.useCaseMaintenance.startUnitOfWork();
		Integer theUseCaseId = this.useCasesGrid.getSelectedRowId();
		if (theUseCaseId != null) {
			...
			if (theResult == IViewConstants.YES) {
				this.useCaseMaintenance.reset();
				this.useCaseMaintenance.deleteUseCase(theUseCaseId);
				this.viewUseCases();
			}
		}
		this.useCaseMaintenance.endUnitOfWork();
