...
public class DownloadAttachment extends SecureServlet {
	...
	public void doGet(HttpServletRequest aRequest, HttpServletResponse aResponse) throws IOException {
		...
		Integer theId = Integer.parseInt(aRequest.getParameter(Attachment.ID));
		String theName = aRequest.getParameter(Attachment.NAME);

		if (super.hasAccess(aRequest, null)) {
			aResponse.setHeader("Content-Disposition", "filename=" + theName);
			aResponse.setHeader("Content-Type", "text/plain");

			AttachmentMaintenance theAttachmentMaintenance = new AttachmentMaintenance();
			byte[] theFile = theAttachmentMaintenance.getAttachmentBy(theId);
